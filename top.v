...
module top(
    ...
    //mux_jal --pcadder and the alu-out
    mux pc_sel_mux_J(
        .sel(JAL_en_top),  //control
        .A(PC_addr_mux_out_top), .B(ALU_out_top),
        .mux_out(JAL_mux_out_top)
    );
 
    //mux_jalr  -- jal  and jalr
    mux pc_sel_mux_Jr(
        .sel(JALr_en_top),  //control
        .A(JAL_mux_out_top), .B(ALU_out_top),
        .mux_out(PC_next_mux_out)  //final pc next
    );
    ...
    //wb mux
    mux date_mem_mux(
        .sel(MemtoReg_top),  //1 - mem data , 0 - alu result --> reg
